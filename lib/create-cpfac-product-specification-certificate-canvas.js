...
export async function createCpfacProductSpecificationCertificateCanvas(
  certificateInfo = {},
  qrcodeContent
) {
  ...
  const CPFAC_FONTS = {
    ...
    khmer_mool1: "Khmer Mool1",
    ang_daunTeav: "Ang DaunTeav"
  };

  const CPFAC_TEMPLATE_IMAGE = "certificate-CPFAC-temporary-baccaluarate.png";
  ...
  const CPFAC_TEXTS = [
    {
      ...
      textSize: 49,
      textStyle: "normal",
      textColor: CPFAC_TEXT_COLORS.blue,
      textFont: CPFAC_FONTS.khmerOSSiemreap,
      y: 688.0402,
      x: 296.5566,
      align: "left",
    },
    {
      ...
      textFont: CPFAC_FONTS.khmerOSMuolLight,
      y: 1049.4019,
      x: 861.5,
      align: "left",
    },
    ...
    },
    {
      ...
      textSize: 48,
      textStyle: "bold",
      textColor: CPFAC_TEXT_COLORS.red,
      textFont: CPFAC_FONTS.TimesNewRomanBold,
      y: 1598.9657,
      x: 929.03,
      align: "left",
    },
    ...
    {
      dataKeys: ["certificate.totalGrade"],
      text: "{certificate.totalGrade}",
      textSize: 46,
      textStyle: "normal",
      textColor: CPFAC_TEXT_COLORS.black,
      textFont: CPFAC_FONTS.khmerOSMuolLight,
  ...
  const ALL_CPFAC_TEXTS = [...CPFAC_TEXTS, ...TechnicalGrades_TEXTS];

  const bg = await createTemplateImage(CPFAC_TEMPLATE_IMAGE);
  const canvas = createCanvas(bg.width, bg.height);
  const ctx = canvas.getContext("2d");
  ctx.drawImage(bg, 0, 0);
  // Draw Caption

  for (const text of ALL_CPFAC_TEXTS) {
    ...
    drawText(canvas, ctx, text.text, {
      x: text.x,
      y: text.y,
      textColor: text.textColor || CPFAC_TEXT_COLORS.black,
      align: text.align,
      font: resolveFont(CPFAC_FONTS.primary, 24, text),
    });
  }
