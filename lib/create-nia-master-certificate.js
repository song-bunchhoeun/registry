...
export async function createNIAMasterCertificate(
  certificateInfo = {},
  qrcodeContent
) {
  const DATA = {
    BACKGROUND: {
      // pngFilename: "nia-master-sample.jpg",
      pngFilename: "nia-master.jpg",
    },
    FONTS: [
  ...
  const majorKmFontSize = adjustFontSize(ctx, nameData[1], 800);
  const nameFontSize = adjustFontSize(ctx, nameData[2], 750);
  const majorFontSize = adjustFontSize(ctx, nameData[3], 1000);
  ...
  if (qrcodeContent) {
    await drawQrCodeStandard(qrcodeContent, ctx, 1613, 1821, 292);
  }
