...
export async function createAUPPMasterCertificate2025(
  certificateInfo = {},
  qrcodeContent
) {
  ...
  const AUPP_TEMPLATE_IMAGE = "aupp-master-summer-205-no-logo.jpg";
  const degree = certificateInfo.certificate.degree;
  let index = -1;
  let result = [degree, ""];

  if (
    certificateInfo.certificate.degree ==
    "Master of Laws in International Business and Digital Technologies"
  ) {
    index = degree.indexOf(" in ");
    if (index !== -1) {
      const firstPart = degree.slice(0, index);
      const secondPart = degree.slice(index + 4);
  ...
  if (
    certificateInfo.certificate.minor &&
    certificateInfo.certificate.minor !== "null"
  ) {
    AUPP_TEXTS.push({
  ...
  if (
    certificateInfo.certificate.honor &&
    certificateInfo.certificate.honor !== "null"
  ) {
    AUPP_TEXTS.push({
