...
export async function createNIEMasterThesisCertificate(
  certificateInfo = {},
  qrcodeContent
) {
  const DATA = {
    BACKGROUND: {
      pngFilename: "nie-master-thesis.jpg",
    },
    FONTS: [
  ...
  const degreeKmFontSize = adjustFontSize(ctx, nameData[0], 520);
  const nameKmFontSize = adjustFontSize(ctx, nameData[1], 1230);
  const secondDegreeKmFontSize = adjustFontSize(ctx, nameData[2], 580);
  const majorKmFontSize = adjustFontSize(ctx, nameData[3], 1240);
  const degreeFontSize = adjustFontSize(ctx, nameData[4], 670);
  const nameFontSize = adjustFontSize(ctx, nameData[5], 1100);
  const secondDegreeFontSize = adjustFontSize(ctx, nameData[6], 1330);
  const majorFontSize = adjustFontSize(ctx, nameData[6], 1270);
