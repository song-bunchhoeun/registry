...
export async function createMPTCDigitalSkillTrainerCompletionCertificateCanvas(
  certificateInfo = {},
  qrcodeContent
) {
  ...
  const MPTC_FONTS = {
    ...
    secondary: "Google Sans Medium",
  };

  const theme = _.get(certificateInfo, "certificate.theme", "");

  const MPTC_TEMPLATE_IMAGE = theme.toLowerCase().includes("training of trainers on digital technology skills for grade 11") ? "mptc-certificate-completion-training-class-11-digital.jpg" : 
    "mptc-certificate-completion-training-digital-no-stamp.jpg";
  ...
  if (qrcodeContent) {
    await drawQrCodeStandard(qrcodeContent, ctx, 1922, 2879, 300);
  }
