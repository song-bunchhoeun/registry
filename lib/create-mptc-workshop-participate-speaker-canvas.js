...
export async function createMptcWorkshopParticipateCanvas(
    certificateInfo = {},
    qrcodeContent
) {
    ...
    const MPTC_TEMPLATE_IMAGE = certificateInfo.recipient.role
        .toLowerCase()
        .includes('a guest speaker')
        ? 'certificate-mptc-workshop-participate-speaker-no-stamp.png'
        : 'certificate-mptc-workshop-participate-coodinator-no-stamp.png';
    ...
    let MPTC_TEXTS = [
        ...
        },
        {
            ...
            text: 'Provincial Departments” {certificate.date}.',
            textSize: 50,
            textColor: MPTC_TEXT_COLORS.black,
            align: 'center',
            y: 2365,
    ...
    for (let i = 0; i < positionLable.length; i++) {
        ...
        if (textWidth.width > thressHoldTextMaxWidth) {
            ...
            positionLable[i].x +=
                (textWidth.width - thressHoldTextMaxWidth) / 2;
        }
    }
