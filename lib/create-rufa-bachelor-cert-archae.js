...
export async function createRUFABachelorCertArchae(
  certificateInfo = {},
  qrcodeContent
) {
  const DATA = {
  ...
  const kmYear = splitedExamDateKm[2].replace("ឆ្នាំ", "");
  ...
  const KM_JUSTIFY_TEXT = [
    {
      ...
      text: "បានឃើញប្រកាសលេខ {certificate.logNoKm} ប្រ.ក ចុះ{certificate.logDateKm} របស់ក្រសួង",
      textSize: 37,
      textColor: "#000000",
      textFont: DATA.FONTS[1],
      x: 379.5,
    ...
    },
    {
      ...
      text: `មហាវិទ្យាល័យ{certificate.degreeKm} ជំនាន់ទី{certificate.generationKm} នាសម័យប្រឡង`,
      textSize: 37,
      textColor: "#000000",
      textFont: { name: "Khmer OS Battambang" },
      x: 379.5,
  ...
  const EN_JUSTIFY_TEXTS = [
    {
      ...
      text: "in accordance with the Prakas Nᵒ {certificate.logNo} dated {certificate.logDate} by the",
      textSize: 40,
      textColor: "#000000",
    ...
    },
    {
      ...
      text: `examinations of the Degree in {certificate.degree}, Generation {certificate.generation}, held on`,
      textSize: 40,
      textColor: "#000000",
      textFont: DATA.FONTS[3],
      x: 1932.6,
  ...
  KM_JUSTIFY_TEXT[1].text += `${kmDay} ${kmMonth} ឆ្នាំ`;
  ...
  EN_JUSTIFY_TEXTS[1].text = EN_JUSTIFY_TEXTS[1].text
    ...
    .replace("Faculty of ", "");
  EN_TEXTS[0][0].text = `${enDay} ${enMonth} ${enYear}, ` + EN_TEXTS[0][0].text;
  ...
  DATA.TEXTS = [...DATA.TEXTS, ...KM_TEXTS, ...EN_TEXTS, ...NUM_TEXTS, ...KM_JUSTIFY_TEXT, ...EN_JUSTIFY_TEXTS];
