...
export async function createMPTCGDAWorkshopLevelFourCertCanvas(certificateInfo = {}, qrcodeContent) {
    ...
    const MPTC_TEMPLATE_IMAGE = 'certificate-training-sihanouk-with-stamp.png';
    const MPTC_TEXTS = [
        {
        ...
        {
            dataKeys: ['certificate.dateKh','certificate.locationKh'],
            text: 'ចាប់ពី{certificate.dateKh} នៅ{certificate.locationKh}។',
            textSize: 50,
        ...
        },
        {
            ...
            text: 'from {certificate.dateEn} at {certificate.locationEn}.',
            textSize: 50,
            textColor: MPTC_TEXT_COLORS.black,
            align: 'center',
            y: 2357,
    ...
    drawPossitionKh(possitionKhStyle, MPTC_TEXT_COLORS.blue, certificateInfo.recipient.positionKh, 1690, MPTC_FONTS, 43, ctx, canvas, bg);
    ...
    drawLongText(possitionEnStyle, MPTC_TEXT_COLORS.blue, certificateInfo.recipient.positionEn, 2174, MPTC_FONTS, 46, 41, ctx, canvas, bg);
    ...
    drawFourCenterText(workshop1Style, MPTC_TEXT_COLORS.black, "ដែលបានចូលរួមសិក្ខាសាលា ស្តីពី ", workshop2Style, MPTC_TEXT_COLORS.black, "“", workshop3Style, MPTC_TEXT_COLORS.black, "ការពង្រឹងសមត្ថភាពអនុវត្តការងារ", workshop4Style, MPTC_TEXT_COLORS.black, "”", 1782, MPTC_FONTS, ctx, canvas, bg)
...
export async function drawLongText(textStyle = {}, textColor, textContent, y, MPTC_FONTS, smallSize, smallSize1, ctx, canvas, bg) {
...
export async function drawPossitionKh(textStyle = {}, textColor, textContent, y, MPTC_FONTS, smallSize, ctx, canvas, bg) {
    ctx.font = `${textStyle.textStyle} ${textStyle.textSize}px ${textStyle.textFont}`;
    let textWidth = ctx.measureText(textContent).width;
    let baseWidth = bg.width - 334;
    if (textWidth > baseWidth) {
...
export async function drawFourCenterText(text1Style = {}, text1Color, text1Content, text2Style = {}, text2Color, text2Content, text3Style = {}, text3Color, text3Content, text4Style = {}, text4Color, text4Content, y, ITC_FONTS, ctx, canvas, bg) {
    ...
    ctx.font = `${text4Style.textStyle} ${text4Style.textSize}px ${text4Style.textFont}`;
    let text4Width = ctx.measureText(text4Content).width;


    let totalContentWidth = text1Width + text2Width + text3Width + text4Width;
    let spaceWidth = (bg.width - totalContentWidth);
    let mulTotalWidth = spaceWidth / 2;
    drawText(canvas, ctx, text1Content, {
        x: mulTotalWidth,
    ...
    drawText(canvas, ctx, text4Content, {
        x: mulTotalWidth + text1Width + text2Width + text3Width,
        y: y,
        textColor: text4Color,
        align: 'left',
        font: resolveFont(ITC_FONTS.primary, 50, text4Style),
    });
