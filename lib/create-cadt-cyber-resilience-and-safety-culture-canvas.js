...
export async function createCADTCyberResilienceAndSafetyCultureCanvas(
    certificateInfo = {},
    qrcodeContent
) {
    ...
    const CADT_TEMPLATE_IMAGE = 'cadt-building-cyber-resilence.jpg';
    // '';
    const CADT_TEXTS = [
        {
            dataKeys: [''],
    ...
    if (certificateInfo.certificate.photoUrl) {
        const profileImage = await loadRemoteResource(
            certificateInfo,
            certificateInfo.certificate.photoUrl
        );
        if (profileImage) {
